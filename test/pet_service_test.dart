...
void main() {
  ...
  group('PetService', () {
    ...
    test('應該正確初始化已保存的寵物狀態', () async {
      ...
      final savedState = PetState(pets: savedPets, equippedPetId: 'MooDeng_R');
    ...
    test('裝備新寵物時應該自動卸下舊寵物', () async {
      ...
      // 確認只有一隻寵物被裝備
      final equippedCount = petService.currentState.pets
    ...
    test('應該正確為所有寵物增加升級點數', () async {
      ...
      final originalPoints = petService.currentState.pets
          .map((pet) => pet.upgradePoints)
          .toList();

      await petService.addUpgradePointsToAll(50);

      final updatedPoints = petService.currentState.pets
          .map((pet) => pet.upgradePoints)
          .toList();
    ...
    test('應該正確計算等級加成倍率', () {
      ...
      expect(
        petService.calculateLevelUpgradeMultiplier(2),
        1.5,
      ); // 假設 base = 0.5
    });
    ...
    test('應該正確取得按稀有度排序的寵物列表', () async {
      ...
      for (int i = 0; i < sortedPets.length - 1; i++) {
        expect(
          sortedPets[i].rarity.sortWeight,
