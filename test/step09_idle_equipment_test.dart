...
void main() {
  ...
  group('Step09 放置裝備整合測試', () {
    ...
    setUpAll(() async {
      ...
      idleIncomeService.init(onIncomeGenerated: (_) {});
    });
    ...
    test('放置裝備解鎖條件驗證', () {
      ...
      expect(
        equipmentService.isIdleEquipmentUnlocked(
      ...
      expect(
        equipmentService.isIdleEquipmentUnlocked(state.equipments, 'youtube'),
        true,
      );
      expect(
        equipmentService.isIdleEquipmentUnlocked(state.equipments, 'btc'),
        true,
      );

      // 先不解鎖 DOGE（需 BTC Lv.5，且後續升級時再用主線解鎖）
      expect(
        equipmentService.isIdleEquipmentUnlocked(state.equipments, 'doge'),
        false,
      );
