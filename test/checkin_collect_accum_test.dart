...
void main() {
  ...
  group('Checkin collect accumulation', () {
    ...
    test('小於 1 的多次累積應轉化為整數進度', () async {
      ...
      expect(
        s1.checkin?.today.task.progress,
        1,
        reason: '0.3 * 5 = 1.5 -> 進度 +1',
      );
      ...
      expect(
        s2.checkin?.today.task.progress,
        6,
        reason: '之前 1 + 0.25*20=5 -> 6',
      );
    });
    ...
    test('非 collect 任務時應忽略並重置緩衝', () async {
      ...
      final newToday = s0.checkin!.today.copyWith(
        task: s0.checkin!.today.task.copyWith(
          type: 'tap',
          target: 5,
          progress: 0,
        ),
      ...
      await gs.updateGameState(
        s0.copyWith(checkin: s0.checkin!.copyWith(today: newToday)),
      );
